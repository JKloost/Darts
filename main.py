...
df = pd.DataFrame({'Pressure': P, 'Water sat': np.array(nu), 'H2O': np.array(H2O), 'CO2': np.array(CO2),
                   'Na+': np.array(Na), 'Cl-': np.array(Cl), 'Halite': np.array(Halite)})
df.to_csv('data.csv')
...
extent = [0, 1, 0, 0.3]
fig, axs = plt.subplots(2, 1, figsize=(10, 7), dpi=400, facecolor='w', edgecolor='k')
...
fig2, axs2 = plt.subplots(2, 2, figsize=(10, 7), dpi=400, facecolor='w', edgecolor='k')
...
im1 = axs2[1][0].imshow(np.array(CO2).reshape(n.nz, n.nx), extent=extent)
axs2[1][0].set_xlabel('x [-]', font_dict_axes)
axs2[1][0].set_ylabel('y [-]', font_dict_axes)
axs2[1][0].set_title('CO2', fontdict=font_dict_title)
divider = make_axes_locatable(axs2[1][0])
cax = divider.append_axes("right", size="5%", pad=0.05)
plt.colorbar(im1, cax=cax)

im2 = axs2[0][1].imshow(np.array(Na).reshape(n.nz, n.nx), extent=extent)
axs2[0][1].set_xlabel('x [-]', font_dict_axes)
axs2[0][1].set_ylabel('y [-]', font_dict_axes)
axs2[0][1].set_title('Na+', fontdict=font_dict_title)
